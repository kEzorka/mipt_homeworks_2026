# ДЗ №1: Budget manager

> [!IMPORTANT]
>
> ## Функции
>
> По заданию давался статический словарь `EXPENSE_CATEGORIES`, в котором значением по **"Other"** был пустой *tuple*.
> Одним из тестов была функция:
>
> ```python
> def category(cls) -> str:
>     random_common_category: str = cls.__random__.choice(tuple(hw3.EXPENSE_CATEGORIES.keys()))
>     random_target_category = cls.__random__.choice(hw3.EXPENSE_CATEGORIES[random_common_category])
>     return f"{random_common_category}::{random_target_category}"
> ```
>
> которая брала рандомный ключ из `EXPENSE_CATEGORIES`, и по этому ключу брала рандомное значение.
>
> Но в случае **"Other"** тесты падали с ошибкой, ведь `cls.__random__.choice(hw3.EXPENSE_CATEGORIES[random_common_category])` не могло взять что-то из ничего.
>
> Поэтому я добавил в **"Other"** подкатегорию *"Other"*
>
> ```python
> "Other": ("Other",),
> ```
>
> ## База данных
>
> По заданию хранение данных осуществляется в
> `financial_transactions_storage`,
> но здесь хранится абсолютно все (и доходы, и убытки).
>
> Поэтому я решил оставить ее только для тестов,
> а логику хранения сделать в
> `database`, чтобы не хранить повторяющиеся элементы.

В рамках данного домашнего задания Вам предстоит реализовать небольшое консольное приложение для управления личными финансами.

Наш потенциальный пользователь хочет вносить данные о своих доходах и расходах, а также получать по ним статистику:

- месячная прибыль/убыток;
- распределение месячных расходов по статьям;
- суммарный размер капитала на конец месяца.

Необходимо с помощью функций реализовать следующие пользовательские сценарии:

## Добавление дохода

Для добавления дохода необходимо обрабатывать строку вида:

``` python
income <amount> <date>
```

Допустим, нам прилетел кэшбэк, целых 49 с чем-то рублей!

``` python
income 49,5 28-02-2026
# или
income 49 28-02-2026
# или
income 49,0 28-02-2026
# или
income 49.0 28-02-2026
```

В ответ печатаем:

``` python
Added
```

Однако доход не может быть ≤ 0. Если ввели неположительное число, выводим ошибку:

``` python
Value must be grater than zero!
```

## Добавление расхода

Для добавления расхода необходимо обрабатывать строку вида:

``` python
cost <category_name> <amount> <date>
```

category_name имеет следующий вид: common_category::target_category

common_category и target_category определены в словаре EXPENSE_CATEGORIES. Пользователь должен иметь возможность узнать все категории по команде

``` python
cost categories
```

А так же увидеть возможные категории, если ввёл не верную при добавлении траты

Вводим

``` python
cost Food::FastFood 555,5 28-02-2026
```

Если всё корректно, печатаем ответ:

``` python
Added
```

Значение расхода не может быть ≤ 0. Если ввели неположительное число, отвечаем:

``` python
Value must be grater than zero!
```

## Просмотр статистики

Сырые данные никому не нужны, мы хотим видеть insights.

Для вывода статистики необходимо обрабатывать строку вида:

``` python
stats <date>
```

Вводим:

``` python
stats 28-02-2026
```

Получаем ответ (на английском):

``` text
Your statistics as of 28-02-2026:
Total capital: -5000.00 rubles
This month, the loss amounted to 5000.00 rubles.
Income: 80000.00 rubles
Expenses: 85000.00 rubles

Details (category: amount):
1. Rent: 45000
2. Gifts: 20,000
3. Subscriptions: 10000
4. Fast food: 10,000
```

Вводим

``` python
stats 15-02-2026
```

Получаем ответ

``` text
Your statistics as of 15-02-2026:
Total capital: 10000.00 rubles
This month, the profit amounted to 10000.00 rubles.
Income: 40000.00 rubles
Expenses: 30000.00 rubles

Details (category: amount):
1. Gifts: 10000
2. subscriptions: 10000
3. fast food: 10,000
```

Обратите внимание:

- Total capital - это сумма всех доходов и расходов до указанной даты
- Статьи расходов сортируются по алфавиту.

- Если расходов в периоде не было - просто не печатаем нумерованный спискок, оставляем строчку с двоеточием.

- Следите за "прибыль составила"/"убыток составил"

## Работа с ошибками

В случае неверных данных, в ответ выводим:

- Если введена неизвестная команда или число аргументов нестандартное: "Unknown command!"
- Если ввели доход или расход меньше или равный нулю: "Value must be grater than zero!"
- Если ввели неверную дату (см. принятые ограничения п.4): "Invalid date!"
- Если в строке неверное число и неверная дата, то выводим ошибку для крайнего левого аргумента (для числа)
- Если ввели не существующую категорию, выводим ошибку и список доступных категорий "Category not exists!"

## Принятые ограничения

1) **Сохранение/чтение данных в файл/бд делать не нужно**. Сериализация/десериализация ждёт нас позже. Исходим из факта, что пользователь при каждом запуске начинает с нуля. То есть нулевой капитал и чистая история.
2) **Названия статей расходов (category_name) состоят из одного слова без пробелов, точек и запятых**. Пустая строка и/или пробелы не принимаются.
3) **Помните про даты доходов и расходов**. В тестах данные могут быть по разным месяцам, в статистиках ориентируемся только на текущий месяц. Исключение - суммарный капитал. Его считаем по всем историческим данным до указанного дня.
4) **Для дат проверка строгая**: Учитывайте високосные года, следите за количеством дней в месяце. (Если год делится на 4, но не делится на 100, он високосный. Однако если год делится на 100, он високосный только в том случае, если делится на 400)
5) **На вход принимаем и целые, и вещественные числа**.
6) **Удалять существующе функции нельзя, менять аргументы тоже. Но добавлять новые для читаемости и переиспользуемости кода - приветствуется.**
7) Импортировать ничего не нужно. Задание нацелено на закрепление работы с циклами, базовыми типами и управляющими конструкциями. datetime использовать тоже запрещается `:)`
8) Тему исключений мы ещё не прошли, поэтому try except тоже не используем.
